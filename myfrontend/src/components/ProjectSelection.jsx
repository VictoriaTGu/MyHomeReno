...
export default function ProjectSelection({ projects, shoppingLists, onSelectProject, onEditShoppingList, isLoading }) {
  if (isLoading) {
  ...
  // Create a map of project_id -> shopping list for quick lookup
  const shoppingListsByProject = {};
  if (shoppingLists && Array.isArray(shoppingLists)) {
    shoppingLists.forEach((list) => {
      if (list.project) {
        shoppingListsByProject[list.project.id] = list;
      }
    });
  }
    ...
    <div className="project-selection">
      ...
      <div className="projects-grid">
        {projectsList.map((project) => {
          const existingList = shoppingListsByProject[project.id];
          return (
            <div key={project.id} className="project-card">
              ...
              {existingList ? (
                <button
                  className="select-btn edit-btn"
                  onClick={() => onEditShoppingList(existingList.id)}
                >
                  Edit Shopping List
                </button>
              ) : (
